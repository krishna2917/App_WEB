    ...
    try {
      if (
        !(
          ...
          originalData instanceof Uint8Array
        )
      )
        return originalData;

      // encrytped binary payload
      var decryptedFrames = await MultiDevice.decryptNoisePacket(
        originalData,
        (isIncoming = false)
      );
      ...
      for (var i = 0; i < decryptedFrames.length; i++) {
        ...
        var realNode = await nodeReaderWriter.decodeStanza(
          decryptedFrameOriginal,
          gzipInflate
        );

        var [isAllowed, manipulatedNode] =
          await NodeHandler.interceptOutgoingNode(realNode);
        ...
        await checkNodeEncoderSanity(
          decryptedFrameOriginal,
          (isIncoming = false)
        );
      }

      var packedNode = await MultiDevice.encryptAndPackNodesForSending(
        decryptedFrames,
        (isIncoming = false)
      );

      var looksEqual = isEqualArray(
        new Uint8Array(originalData),
        new Uint8Array(packedNode)
      );
      ...
      if (isInitializing) {
        ...
        document.dispatchEvent(
          new CustomEvent("onInterceptionWorking", {
      ...
      console.error(
        "WhatsIncognito: Passing-through outgoing packet due to exception:"
      );
  ...
  var promise = async function (messageEvent) {
    ...
    try {
      ...
      for (var i = 0; i < decryptedFrames.length; i++) {
        ...
        var [isAllowed, manipulatedNode] =
          await NodeHandler.interceptReceivedNode(realNode);
        ...
        decryptedFrames[i] = {
          ...
          counter: counter,
          decryptedFrame: decryptedFrame,
        };
      }

      var packet = await MultiDevice.encryptAndPackNodesForSending(
        decryptedFrames,
        true
      );
      ...
      if (exception.message && exception.message.includes("stream end"))
        return messageEvent;
      if (typeof exception == "string" && exception.includes("counter")) {
        console.log(exception);
        return messageEvent;
...
function onDeletionMessageBlocked(
  message,
  remoteJid,
  messageId,
  deletedMessageId
  ...
  var messageNode = document.querySelector(
    "[data-id*='" + deletedMessageId + "']"
  );
  if (messageNode) {
  ...
  setTimeout(async function () {
    ...
    if (chat) {
      ...
      else await WhatsAppAPI.LoadEarlierMessages.loadEarlierMsgs(chat);
...
async function decryptE2EMessagesFromNode(node) {
  ...
  try {
    ...
    console.error(
      "Could not decrypt E2E message with type " +
        node.attrs["type"] +
        " due to exception:"
    );
    console.error(exception);
...
async function interceptViewOnceMessages(e2eMessage, messageId) {
  if (
    e2eMessage.viewOnceMessageV2 !== null ||
    e2eMessage.viewOnceMessageV2Extension !== null
  ) {
    var retrievedMsg = {};
    var type = "";
    if (e2eMessage.viewOnceMessageV2 !== null) {
      ...
      e2eMessage.viewOnceMessageV2Extension?.message?.audioMessage !== null
    ) {
      retrievedMsg = e2eMessage.viewOnceMessageV2Extension.message.audioMessage;
      type = "audio";
    } else {
      throw new Error(
        "Unknown viewOnceMessageV2 or viewOnceMessageV2Extension type"
      );
    }
    const mediaKeyEncoded = btoa(
      String.fromCharCode.apply(null, retrievedMsg.mediaKey)
    );
    const encodedencFileHash = btoa(
      String.fromCharCode.apply(null, retrievedMsg.fileEncSha256)
    );
    const encodedfileSha256 = btoa(
      String.fromCharCode.apply(null, retrievedMsg.fileSha256)
    );

    if (window.WhatsAppAPI !== undefined) {
      const decryptedData =
        await WhatsAppAPI.downloadManager.downloadAndMaybeDecrypt({
          directPath: retrievedMsg.directPath,
          encFilehash: encodedencFileHash,
          filehash: encodedfileSha256,
      ...
      viewOnceDBOpenRequest.onsuccess = () => {
        ...
        var viewOnceRequest = viewOnceTranscation
          .objectStore("msgs")
          .add({ id: messageId, dataURI: dataURI, caption });
        viewOnceRequest.onerror = (e) => {
          if (viewOnceRequest.error.name == "ConstraintError") {
            if (WAdebugMode) {
              console.log(
                "WhatsIncognito: Not saving message becuase the message ID already exists"
              );
            }
          } else {
            console.warn(
              "WhatsIncognito: Unexpected error saving deleted message"
            );
          }
        };
      };
...
function printNode(node, isIncoming = false, decryptedFrameLength) {
  ...
  if (isIncoming) {
    console.log(
      "[In] Received binary (" + decryptedFrameLength + " bytes, decrypted)): "
    );
  } else {
    console.log(
      "[Out] Sending binary (" + decryptedFrameLength + " bytes, decrypted): "
    );
  }
...
function exposeWhatsAppAPI() {
  ...
  if (window.require) {
    // React Native
    window.WhatsAppAPI.downloadManager =
      require("WAWebDownloadManager").downloadManager;
    window.WhatsAppAPI.ChatCollection =
    ...
    window.WhatsAppAPI.sendPresenceStatusProtocol =
      require("WASendPresenceStatusProtocol").sendPresenceStatusProtocol;
    ...
    window.WhatsAppAPI.downloadManager =
      moduleFinder.findModule("downloadManager")[0].downloadManager;
    window.WhatsAppAPI.ChatCollection =
      moduleFinder.findModule("Msg")[0].default.Chat;
    window.WhatsAppAPI.Seen = moduleFinder.findModule("sendSeen")[0];
    window.WhatsAppAPI.Communication = moduleFinder
      .findModule("getComms")[0]
      .getComms();
    window.WhatsAppAPI.LoadEarlierMessages =
      moduleFinder.findModule("loadEarlierMsgs")[0];
    window.WhatsAppAPI.sendPresenceStatusProtocol = moduleFinder.findModule(
      "sendPresenceStatusProtocol"
    )[0].sendPresenceStatusProtocol;
    window.WhatsAppAPI.SignalStore = moduleFinder.findModule(
  ...
  if (window.WhatsAppAPI.Seen == undefined) {
    console.error(
      "WhatsAppWebIncognito: Can't find the WhatsApp API. Sending read receipts might not work."
    );
  }
}
...
function hookLogs() {
  ...
  Object.defineProperty(window, "onunhandledrejection", {
    set: function (value) {
      originalOnUnhandledRejection = value;
    },
    get: function () {
      return hookedPromiseError;
    },
  });
...
async function saveDeletedMessage(
  retrievedMsg,
  deletedMessageKey,
  revokeMessageID
) {
  ...
  if ("id" in deletedMsgContents) {
    const transcation = window.deletedMessagesDB.transaction(
      "msgs",
      "readwrite"
    );
    ...
    request.onsuccess = (e) => {
      console.log(
        "WhatsIncognito: Saved deleted message with ID " +
          deletedMsgContents.id +
          " from " +
          deletedMsgContents.from +
          " successfully."
      );
    };
  } else {
...
async function checkNodeEncoderSanity(originalFrame, isIncoming = false) {
  ...
  if (flags & 2) {
    ...
    decryptedFrameOpened = toArrayBuffer(
      pako.inflate(new Uint8Array(decryptedFrameOpened))
    );
  }
  ...
  // sanity check that our node parsing is deterministic
  var encodedNodeData = await nodeReaderWriter.encodeStanza(
    realNode,
    isIncoming
  );
  var looksGood = isEqualArray(
    new Uint8Array(decryptedFrameOpened),
    encodedNodeData.slice(1)
  );
